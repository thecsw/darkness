...
// with any fatal errors (if occurred).
func DownloadImage(link string, authority, prefix, name string) (image.Image, error) {
	...
	if _, err := io.Copy(io.MultiWriter(buf, bar), resp.Body); err != nil && !errors.Is(err, io.EOF) {
		return nil, fmt.Errorf("reading downloaded image data: %w", err)
	}
